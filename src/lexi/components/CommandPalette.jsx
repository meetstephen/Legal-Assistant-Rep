...
import {
  Search, CornerDownLeft, Command, Globe, Moon, Sun, MessageCircle, LogOut,
} from 'lucide-react';
...
export function CommandPalette() {
  const {
    navigate, webGrounding, setWebGrounding, isDark, toggleTheme,
    supabaseEnabled, signOut, showToast,
    // isAdmin is a first-class context value — never read from profile
    isAdmin,
  } = useApp();
  ...
  const commands = useMemo(() => {
    ...
    const actions = [
      ...
      },
      {
        ...
        run: () => {
          ...
          showToast('success', `Web grounding ${webGrounding ? 'off' : 'on'}.`);
        },
      },
      {
        id: 'act:theme',
    ...
    if (supabaseEnabled) {
      actions.push({
        ...
        hint: 'Account',
        icon: LogOut,
        run: async () => { await signOut(); showToast('info', 'Signed out.'); },
      });
    }
    return [...pages, ...actions];
  }, [isAdmin, webGrounding, isDark, supabaseEnabled, navigate, setWebGrounding, toggleTheme, signOut, showToast]);
    ...
    <div className="fixed inset-0 z-[70] flex items-start justify-center pt-[12vh] px-4">
      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-modalFadeIn">
        <div className="flex items-center gap-3 px-4 border-b border-slate-200 dark:border-slate-800">
          ...
          />
          <kbd className="hidden sm:flex items-center gap-1 text-[10px] text-slate-400 border border-slate-300 dark:border-slate-600 rounded px-1.5 py-0.5">
            ESC
          </kbd>
        </div>
