...
// Maps a public.profiles row (snake_case, Supabase shape) to the camelCase
// row shape the existing UI already renders.
function mapProfileToUserRow(p) {
  return {
    ...
    // Real Supabase Auth accounts can't be deleted from the client (no
    // service-role key in the browser, by design). Manage account removal
...
// Single source of truth for the admin user list.
// Cloud mode (SUPABASE_ENABLED): reads every real registered account from
// public.profiles via RLS — this is what makes other users' signups visible
// to the admin, across devices and browsers.
// Local mode (no Supabase configured): falls back to the original
// localStorage-seeded list, preserving single-device/dev behaviour.
function useAdminUsers() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Most common cause: RLS denied (logged-in account isn't in the admin
      // email allow-list in supabase/schema.sql) or `profiles` table/migration
      // hasn't been run yet.
...
function UsersTab({ showToast, audit }) {
  ...
  // Local-mode-only persistence. In cloud mode, writes go straight to
  // Supabase (see toggleStatus) and `users` is refreshed from there instead.
  ...
  const addUser = () => {
    if (SUPABASE_ENABLED) {
      showToast('warning', 'Real accounts sign up themselves, or invite them from the Supabase Dashboard → Authentication → Users → Invite.');
      return;
    }
    ...
    <div className="space-y-4">
      {SUPABASE_ENABLED && !error && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 text-sm text-blue-800 dark:text-blue-200">
          <strong>Cloud mode:</strong> Showing every registered account from Supabase. New signups, add
          via the <a href="https://supabase.com/dashboard" target="_blank" rel="noopener noreferrer" className="underline">Supabase Dashboard</a> →
          Authentication → Users → Invite. Suspending a user here flags their status — it does not block
          sign-in (that requires the Dashboard's "Ban user" action).
        </div>
      )}
      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 text-sm text-red-700 dark:text-red-300">
          <strong>Couldn't load users:</strong> {error}. If this is the first time, make sure you've run the
          updated <code className="font-mono">supabase/schema.sql</code> in the Supabase SQL editor.
          <Button variant="outline" size="sm" className="mt-2" onClick={refresh}>Retry</Button>
        </div>
      )}
      {loading && !error && (
...
export function Admin() {
  ...
  // Update seed admin last-login timestamp on mount (local mode only — cloud
  // mode tracks real last_login via touchOwnProfile in AppContext.jsx).
  useEffect(() => {
    if (!isAdmin || SUPABASE_ENABLED) return;
